...
Data from Siegel 2014, Henson 2012, Laws 2000, Laws 2011 and Dunne 2014 are in the same mat file, but Siegel 2014 has a slightly different coordinates grid.

```{r siegel_2014}
mat_calipso <- readMat("data/raw/export4CALIPSO.mat")
...
df_sieg_2014 <- as.data.frame(mat_sieg) %>%
  ...
  mutate(
    ...
    depth = 100,
    poc_flux = na_if(poc_flux, 0),  # Convert 0 to NA ,
    poc_flux = poc_flux / 365.25,  # Convert POC flux from mg C m⁻² y⁻¹ to mg C m⁻² d⁻¹
...
df_others <- map_dfr(calipso_models$target, function(targ) {
...
Are these data really mg C m⁻² y⁻¹? It looks like it’s 1  order of magnitude too high!
:::
...
df_list <- list(df_wang_2023, df_now_2022, df_sch_2000, df_sieg_2014, df_others)

# Let’s purrr!
g_obs_flux <- map_dfr(df_list, ~ .x %>%
  mutate(
    # round lon and lat
