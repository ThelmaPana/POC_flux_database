---
title: "Read and format POC flux from models in Doney et al. 2024"
...
mod_name_100 <- tibble(
  ...
  mutate(colour = color("discreterainbow")(n())) # generate colours for models
...
mod_name_1000 <- tibble(
  ...
  arrange(model) %>%  # sort alphabetically
  left_join(mod_name_100 %>% select(model, colour), by = join_by(model)) # keep the same colour as in models at 100 m
...
Save POC flux data from models.

```{r save}
mod_flux <- bind_rows(
  # assemble data at 100 and 100 m
  mod_flux_100 %>% mutate(depth = 100),
  mod_flux_1000 %>% mutate(depth = 1000)  
) %>% 
  mutate(depth = as.factor(depth)) # depth as factor
  
save(mod_flux, file = "data/00.mod_flux.Rada")
```
