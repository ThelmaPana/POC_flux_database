...
```{r read_obs}
obs_flux <- read_csv("data/raw/Global_POC_Database_Simon_Cleaned.csv", show_col_types = FALSE) %>% 
  # select relevant columns
  select(lon = longitude_final, lat = latitude_final, depth = `depth, water [m]`, datetime = `date/time`, poc_flux_obs = `carbon, organic, particulate, flux [mg/m**2/day]`)
...
::: callout-note
For 1000 m, there is a strong increase in the number of observations (× 3.25) when considering the range 800 - 1200 m instead of 1000 m. Let’s thus use the 20% range for both 100 and 1000 m.
:::
...
```{r save}
save(obs_flux_targ, file = "data/01.obs_flux_targ.Rdata")
```
